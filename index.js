...
function sendSearch(message, replyToken) {
  ...
  return replyText(replyToken, result[0].href);
}
...
function gameOX(author, message, replyToken) {
  ...
  manager.game[0].tictactoe.forEach(g=> {
    if (g.id == player.id) { g.board = [0, 1, 2, 3, 4, 5, 6, 7, 8]; board = g.board; }
  })
...
return new Promise((res, rej) => {
  ...
  .then((ids) => {
    ids.forEach((id) => mention.push([uid:{id:id}]));
  })
  .catch((err) => {
    console.log(err)
  });
  mention.uid.forEach((id) => {
    client.getGroupMemberProfile(groupId, userId)
    .then((profile) => {
      if (id == profile.userId) {
        mention.uid.push({username:profile.displayName});
        mention.uid.push({avatarURL:profile.pictureUrl});
        mention.uid.push({status:profile.statusMessage});
      }
    })
    .catch((err) => {
    console.log(err)
...
function onCommand(author, message, replyToken) {
  ...
  if (cmd === command.avatar[0].text || cmd === command.avatar[1].text) {
    ...
    if (author.gid != "") {
      var mention = await isMention(author, message);
      if (mention) {
        mention.uid.forEach((m) => {
