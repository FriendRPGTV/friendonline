...
let fukgerMessage = ["ควย","หี","แตด","เย็ดแม่","เย็ด","ค.วย","สัส","เหี้ย","ไอ้เหี้ย","มึงตาย","ไอ้เลว","ระยำ","ชาติหมา","หน้าหี","กาก","พ่องตาย","ส้นตีน","แม่มึงอ่ะ","แม่มึงดิ","พ่อมึงดิ"];
...
function handleMessage(message, replyToken) {
  ...
  if (msg.startsWith(prefix)) {
    onCommand(message, replyToken);
  } else {
    onChat(message, replyToken);
  }
}

function sendAvatar(message, replyToken) {
  Promise
  .resolve(message.author.avatarURL)
  .then((ImageUrl) => replyImage(replyToken, [ImageUrl, ImageUrl]));
}
...
function sendSearch(message, replyToken) {
  ...
  google(str, (err, res) => {
    ...
    for (var i = 0; i < res.links.length; ++i) {
      ...
      result.push([link.title, link.href])
    }
  })
  console.log(result);
  return replyText(replyToken, result);
...
function sendGame(message, replyToken, game) {
  switch (game) {
    case 'ox':
      return gameOX(message, replyToken);
    case 'rpc':
      return gameRPC(message, replyToken);
    default:
      throw new Error(`ไม่มีเกมส์ ${game} ในระบบค่ะ: ${JSON.stringify(message)}`);
  }
}

function gameOX(message, replyToken) {
  ...
  let title = [`เกมส์ OX | OX Game`, 'https://i.imgur.com/9V8XTgm.png'];
  let des = `พิมพ์ ตัวเลข เพื่อวาด ⭕ ลงในช่องว่าง`;
  let turn = `รอบของคุณ${player.username}`;
...
function drawOX(message, replyToken) {
  var img;
  ...
  for (let i = 0; i < 9; i++) {
    if (num == i) {
      ...
      updateBoard(user.id, origBoard);if (!winning(origBoard, huPlayer)) {
...
function onChat(message, replyToken) {
  let msg = message.text;
  if (checkType(message.author.type)) {
    if (!msg.includes(botname)) return;
    lunaChat(message, replyToken);
  } else {
    if (inputOX(msg)) return drawOX(message, replyToken);
    lunaChat(message, replyToken);
  }
}

function lunaChat(message, replyToken) {
  ...
  if (reply && say.includes(reply.ask)) {
    ...
    console.log(`from: ${message.author.username} say: ${message.text}`);
    const embed = {
      'content': `**Line Message Api**\nfrom: ${message.author.username}\nID: ${message.author.id}\nMessage: ${message.text}`
    };
    //webhooks(embed);
  }
}
...
function onCommand(message, replyToken) {
  let sender = message.author;
  let name = sender.username; 
  ...
  if (cmd === command.help[0].text || cmd === command.help[1].text) {
    let text = [
      ...
      `- คำสั่งส่งรูปอนิเมะตบ (${prefix}${command.pat[0].text} หรือ ${prefix}${command.pat[1].text})`,
      `- คำสั่งส่งรูปอนิเมะเลีย (${prefix}${command.slap[0].text} หรือ ${prefix}${command.slap[1].text})`,
      ...
      //`- คำสั่งเล่นเกมส์เปายิ่งฉุบกับบอท (${prefix}${command.rpc[0].text} <rock/paper/crisis> หรือ ${prefix}${command.rpc[1].text} <ค้อน/กระดาษ/กรรไกร>)`,
  ...
  if (cmd === 'eval') {
    if (message.user.id !== chaID) return;
    eval(arg.join(' '));
  }
}
...
function handleText(message, replyToken, source) {
  ...
  .then((profile) => {
    console.log('[Profile]');
    console.log(profile);
    message.author.username = profile.displayName;
    message.author.type = source.type;
    message.author.id = source.userId;
    if (source.type === 'user') {
      message.author.status = profile.statusMessage;
      message.author.avatarURL = profile.pictureUrl;
    }
  });
  switch (message.text) {
    case 'profile':
      if (source.userId) {
        return replyText(replyToken, [`ชื่อ: ${message.author.username}`, `สถานะ: ${message.author.status}`]);
