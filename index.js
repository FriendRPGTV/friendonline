...
var manager;// = LunaClient.manager;
const config = {
  channelAccessToken: process.env.TOKEN,
  channelSecret: process.env.SECRET,
};
manager = luna.database({name: 'load'});
...
function sendCanvas(player, canvas, replyToken, board = []) {
    var out, stream, img, title, play = `${board[0]}${board[1]}${board[2]}${board[3]}${board[4]}${board[5]}${board[6]}${board[7]}${board[8]}`, part = '/downloaded/'+`ttt_${player.id}_${play}.png`;
...
return new Promise((res, rej) => {
  ...
  mention.forEach((m) => {
    ...
    .then((profile) => {
      if (m.id == profile.userId) {
        m.push({username:profile.displayName});
        m.push({avatarURL:profile.pictureUrl});
        m.push({status:profile.statusMessage});
        mention[m.id] = m;
      }
    })
    .catch((err) => {
  ...
  switch (message.text) {
    ...
    default:
      ...
      .then((profile) => {
        ...
        if (!replyToken) return pushText(source.groupId, 'ขออภัยด้วยค่ะ คุณไม่สามารถใช้คำสั่งได้ในตอนนี้ค่ะ กรุณาเพิ่มเฟรนเป็นเพื่อนด้วยนะคะ 👉 ');
