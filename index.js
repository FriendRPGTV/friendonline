...
function sendCanvas(player, canvas, replyToken) {
    var out, stream, img, part = './downloaded/'+`ttt_${player.id}.png`;
    out = fs.createWriteStream(part);
    stream = canvas;
    stream.on('data', function (chunk) {
        console.log("Write chunk for", part)
        out.write(chunk);
    });
    stream.on('end', function () {
        console.log("End stream for", part)
    });
    try {
      img = new Promise((resolve, reject) => {
        console.log('Part: ', part);
        const read = fs.createReadStream(path)
        read.on('data', (chunk) => read.pipe(chunk));
        read.on('end', () => resolve(path));
  ...
  /*let getContent;
  const downloadPath = './downloaded/'+`ttt_${player.id}.png`;
  getContent = loadImages(canvas, downloadPath)
    .then((download) => {
      let uri = loadImage(download);
      return {
...
function loadImage(url) {
  ...
  /*out = fs.createWriteStream(__dirname + '/' + url), 
