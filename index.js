...
async function sendYoutube(author, message, replyToken) {
  ...
  if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
    ...
    try {
        ...
        `${videos.map((video)=>`[${++index}] ${video.title} | เวลา ${(video.duration.hours > 0 ? video.duration.hours+':' : '')}${(video.duration.minutes > 0 ? (video.duration.minutes > 9 ? video.duration.minutes+':' : '0'+video.duration.minutes+':') : '')}${(video.duration.seconds > 9 ? video.duration.seconds : '0'+video.duration.seconds)}`).join('\n')}`
        ];
        imgcaroucel = {
          type: 'template',
          altText: 'Youtube '+searchString,
