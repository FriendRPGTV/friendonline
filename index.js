...
function onCommand(author, message, replyToken) {
  ...
  if (cmd === 'm') {
    let link = args.join(' ');
    if (!link.startsWith('https://video.mthai.com/general/player/')) return replyText(replyToken, 'ขออภัยด้วยค่ะ ดูเหมือนว่าลิ้งของคุณไม่ถูกต้อง โปรดใช้ลิ้งที่ขึ้นต้นด้วย https://video.mthai.com/general/player/{id}.html');
    https.get(link, (res) => {
      res.on('data', (chunk) => {
        text += chunk;
      })
        ...
        .then((profile) => {
         ...
         jimp.read('https://i.imgur.com/a1rD6tS.png', (err, ava) => {
          ...
          ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
           ...
           ctx.register('font.ttf', { family: 'font'})
           ...
           jimp.read(author.avatarURL, (err, img) => {
            ...
            img.getBuffer(jimp.MIME_PNG, (err, buff) => {
             ...
             stream.on('data', (chunk) => out.write(chunk))
             stream.on('end', () => {
              return client.pushMessage(author.id, { type: 'image', originalContentUrl: baseURL+'/static/'+`profile_${author.id}_${love.point}.png`, previewImageUrl: baseURL+'/static/'+`profile_${author.id}_${love.point}.png`})
             })
            })
           })
          })
