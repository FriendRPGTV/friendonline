...
async function sendYoutube(author, message, replyToken) {
  ...
  if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
    ...
    try {
        ...
        `${videos.map((video2)=>`[${++index}] ${video2.title} | เวลา ${(video2.duration.hours > 0 ? video2.duration.hours+':' : '')}${(video2.duration.minutes > 0 ? (video2.duration.minutes > 9 ? video2.duration.minutes+':' : '0'+video2.duration.minutes+':') : '')}${(video2.duration.seconds > 9 ? video2.duration.seconds : '0'+video2.duration.seconds)}`).join('\n')}`
        ];
        imgcaroucel = {
          type: 'template',
          altText: 'Youtube '+searchString,
...
function sendSearch(author, message, replyToken) {
  ...
  google(str, (err, res) => {
    ...
    for (var i = 0; i < index.length; ++i) {
      const link = index[i];
      caroucel.template.columns[i].thumbnalImageUrl = 'https://i.imgur.com/KHNJRhm.jpg';
